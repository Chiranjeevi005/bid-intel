...
const runName = 'build-008n';
const strategy = process.env.EXTRACTION_STRATEGY || 'N1';
const outDir = path.join(process.cwd(), 'benchmarks', runName, 'results', strategy);
...
async function llmJudgeExtractionMiss(goldFact: string, modelFindings: any[]): Promise<boolean> {
    const apiKey = process.env.DEEPSEEK_API_KEY;
    if (!apiKey || modelFindings.length === 0) return false;

    const prompt = `
You are a benchmark evaluator.
We are checking if an AI model extracted a specific fact, even if it worded it differently.
...
Did the model output findings contain the core concept of the gold fact? 
Answer strictly in JSON: {"concept_present": true/false}
`;
    try {
...
async function main() {
    ...
    let misses = {
        ...
        TRUE_EXTRACTION_MISS: 0
    };

    console.log(`Evaluating BUILD-008N Benchmark (Strategy: ${strategy})\n`);

    let failureLog = "";
    let falseNegativeLog = "";
    ...
    for (const doc of manifest) {
        ...
        for (const gold of goldItems) {
            ...
            if (extractionSuccess) {
                ...
                // Diagnostic Semantic Recall
                const semanticPresent = await llmJudgeExtractionMiss(gold.fact, modelItems);
                if (semanticPresent) {
                    misses.EVALUATOR_FALSE_NEGATIVE++;
                    falseNegativeLog += `[DOC: ${doc.id}] EVALUATOR_FALSE_NEGATIVE: "${gold.fact}"\n`;
                } else {
                    misses.TRUE_EXTRACTION_MISS++;
                }

                failureLog += `[DOC: ${doc.id}] EXTRACTION_MISS: "${gold.fact}" (Page ${gold.page_number}) (SemanticPresent: ${semanticPresent})\n`;
                continue;
            }
    ...
    if (fs.existsSync(globalTelPath)) {
        ...
        agg.identicalRetries = globalTel.stats?.totalIdenticalRetries || 0;
    }
    ...
    const semCritRec = ((agg.criticalRecalled + misses.EVALUATOR_FALSE_NEGATIVE) / agg.criticalGoldItems * 100).toFixed(1);

    console.log(`| **Critical Recall**           |      **≥90%** |  ${critRec}% | ${Number(critRec) >= 90 ? 'PASS' : 'FAIL'} |`);
    console.log(`| Secondary Semantic Recall     |               |  ${semCritRec}% | - |`);
    ...
    console.log(`| **p90 E2E**                   |      **≤90s** |  ${p90Latency.toFixed(1)}s | ${strategy === 'N4' ? 'N/A' : (p90Latency <= 90 ? 'PASS' : 'FAIL')} |`);
    console.log(`| **Max E2E**                   | ideally ≤120s | ${maxLatency.toFixed(1)}s | - |`);

    console.log(`\n### Decomposition of Misses`);
    console.log(`Total Misses (Deterministic): ${agg.totalGoldItems - agg.overallRecalled}`);
    console.log(`- RETRIEVAL_MISS:           ${misses.RETRIEVAL_MISS}`);
    console.log(`- EXTRACTION_MISS (Total):  ${misses.EXTRACTION_MISS}`);
    console.log(`    ↳ TRUE_EXTRACTION_MISS:     ${misses.TRUE_EXTRACTION_MISS}`);
    console.log(`    ↳ EVALUATOR_FALSE_NEGATIVE: ${misses.EVALUATOR_FALSE_NEGATIVE}`);
    ...
    if (failureLog.length > 0) {
        ...
        console.log(`\nDetailed failure log saved to benchmarks/build-008n/results/${strategy}/failure_log.txt`);
    }
}
